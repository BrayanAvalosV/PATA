...
const fetchList = async (tipo, limit) => {
  const url = `${API}/api/mascotas?tipo=${encodeURIComponent(tipo)}${limit ? `&limit=${limit}` : ""}&_=${Date.now()}`;
  try {
      ...
      {items === null ? (
        <section className="py-16 bg-gray-50">
          <div className="max-w-7xl mx-auto px-6">
            <h2 className="text-4xl font-bold text-center text-green-700 mb-10">Mascotas en Adopción 🐶🐱</h2>
    ...
    <>
      {items === null ? (
        <section className="py-16 bg-amber-50">
          <div className="max-w-7xl mx-auto px-6">
            <h2 className="text-4xl font-bold text-center text-amber-600 mb-10">Mascotas Extraviadas 🔍</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
              {[...Array(6)].map((_, i) => (
                <div key={i} className="bg-white shadow-md rounded-2xl p-4 animate-pulse h-80" />
              ))}
  ...
  return (
    <div className="font-sans bg-gray-50 min-h-screen">
      ...
      <Routes>
        ...
        <Route path="/publicar" element={<ProtectedRoute><><PublicarMascota /><Footer /></></ProtectedRoute>} />
        <Route path="/post/:id" element={<><DetalleMascota /><Footer /></>} />
